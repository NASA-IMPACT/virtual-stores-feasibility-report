...
## Non-cubable/Swath/Tiled collections

Virtualizing "non-cubable" satellite collections exposes significant limitations when attempting to unify datasets. The current challenges are outlined below.

:::{.callout-note}
"Non-cubable" datasets are ones that can't be neatly organized into a single, regular data cube. This usually happens because different parts of the data use different coordinate systems or have overlapping/irregular grids.

Non-cubable datasets are often labeleed Level 2 (L2). They are often or swath-based data, where observations are collected in strips rather than on a consistent grid. These datasets are often organized into "tiles" (e.g. Sentinel) or "frames" (e.g. OPERA and NISAR).
:::

### Current challenges for non-cubable data
