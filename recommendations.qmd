...
For example, time series analysis is a common use case. However, many datasets are collected into one file per timestep. This makes sense, since data is collected over time: generating one file at a time makes data processing straight-forward since there is no appending to an existing file. However, this means time series will never be optimized since data for one timeseries is scattered across n-timesteps number of chunks. Designing how files are chunked is a decision that should made deliberately, with awareness of the trade-offs for other access patterns.

## Adopt icechunk

[Icechunk](icechunk.io) should be adopted but with risk mitigation measures. While open source, this technology is maintained by a small and external development team. This introduces a risk which is a dependency on that external development team. NASA should mitigate this risk by funding icechunk maintenance and development. 

More specifically, NASA should fund:
...
* Icechunk maintenance (both the existing development team and new developers)
