...
Currently, Harmony users can request transformation of all files in a data product. For most services this means parallelizing the processing of multiple inputs in different Kubernetes pods. Aggregation may occur but, generally, a single pod only needs to download, process and output a single object with a comparable size to an archival data file. With a whole-collection virtual data store, a single Harmony invocation would conduct processing and construction of an output in one Kubernetes pod. Solutions for parallel processing exists, such as dask. However, if a single file-like output object is requested, an output file object could become too large for tradition download-based workflows.
